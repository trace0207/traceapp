...
-(BOOL)shouldTap:(BOOL)select
{
    if (_delegate!=nil&&[_delegate respondsToSelector:@selector(shouldSelectAsset:selected:)]) {
        return [_delegate shouldSelectAsset:_asset selected:select];
    }
    return YES;
}
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    ZYQAssetGroupViewController *vc = (ZYQAssetGroupViewController *)self.navigationController.viewControllers[0];
    [vc.tableView setContentOffset:CGPointMake(0, vc.tableView.contentSize.height-vc.tableView.frame.size.height) animated:NO];
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:(self.tableView.frame.size.width-kThumbnailSize.width*columns-minimumInteritemSpacing*(columns-1))/2];
    return cell;
}
...
- (BOOL)shouldSelectAsset:(ALAsset *)asset selected:(BOOL)selected
{
    ...
    //zxx
    if (!selected && (_indexPathsForSelectedItems.count >= vc.maximumNumberOfSelection)) {
        NSNumber *numble = [NSNumber numberWithInteger:vc.maximumNumberOfSelection];
        NSString *msg = [NSString stringWithFormat:@"最多选择%@张图片",[numble stringValue]];
