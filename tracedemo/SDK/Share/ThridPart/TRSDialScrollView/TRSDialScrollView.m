...
@property (strong, nonatomic) UIView *tickView;
...
- (void)commonInit
{
    ...
    _tickView = [[UIView alloc]initWithFrame:CGRectMake(self.frame.size.width/2 - 1, self.frame.size.height - self.majorTickLength, 2, self.majorTickLength)];
    _tickView.backgroundColor = [UIColor redColor];
    _tickView.userInteractionEnabled = NO;
...
- (CGPoint)scrollToOffset:(CGPoint)starting {
    ...
    _currentValue = ending.x/self.dialView.minorTickDistance+self.min;
    
    
    if (self.dialDelegate && [_dialDelegate respondsToSelector:@selector(dialEndScrollView:WithValue:)]) {
        [_dialDelegate dialEndScrollView:self WithValue:_currentValue];
    }
...
- (void)setMajorTickLength:(CGFloat)majorTickLength
{
    ...
    self.tickView.frame = CGRectMake(self.frame.size.width/2 - 1, self.frame.size.height - self.majorTickLength-1, 2, self.majorTickLength+1);
}
...
- (void)setOverlayColor:(UIColor *)overlayColor
{
    ...
    self.tickView.backgroundColor = overlayColor;
}
