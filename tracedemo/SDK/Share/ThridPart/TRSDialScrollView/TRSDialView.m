...
- (void)drawLabelWithContext:(CGContextRef)context
    ...
    [text drawInRect:CGRectMake(label_x, point.y + 7, boundingBox.width, boundingBox.height)
...
- (void)drawTicksWithContext:(CGContextRef)context atX:(int)x
{
    ...
    CGPoint majorPoint = CGPointMake(x, self.superview.frame.size.height-self.majorTickLength);
    CGPoint minorPoint = CGPointMake(x, self.superview.frame.size.height-self.minorTickLength);
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (NSInteger i = self.leading; i < rect.size.width; i += self.minorTickDistance) {
        ...
        // Middle
            ...
            [self drawTicksWithContext:context atX:(int)i];
