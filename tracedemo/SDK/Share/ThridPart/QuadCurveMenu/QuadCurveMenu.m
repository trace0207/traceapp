...
#define NEARRADIUS 140.0f
#define ENDRADIUS 145.0f
#define FARRADIUS 150.0f
#define STARTPOINT CGPointMake(self.frame.size.width-50.0f, self.frame.size.height-50.0f)
#define TIMEOFFSET 0.0097f
...
- (id)initWithFrame:(CGRect)frame menus:(NSArray *)aMenusArray
{
    ...
    if (self) {
        ...
        // add the "Add" Button.
        _addButton = [[QuadCurveMenuItem alloc] initWithImage:[UIImage imageNamed:@"choose_button"]
                                       highlightedImage:[UIImage imageNamed:@"choose_button"]
                                           ContentImage:[UIImage imageNamed:@"choose_button"]
                                highlightedContentImage:[UIImage imageNamed:@"choose_button"]];
...
- (void)setExpanding:(BOOL)expanding
{
    ...
    // rotate add button
    float angle = self.isExpanding ? -M_PI+0.000001 : 0.0f;
    [UIView animateWithDuration:0.2f animations:^{
        _addButton.transform = CGAffineTransformMakeRotation(angle);
    }];
...
- (void)_expand
{
    if (_flag == 6)
    {
       ...
       // [_timer release];
        _timer = nil;
        if (_delegate && [_delegate respondsToSelector:@selector(quadCurveMenuTouchesBeganExpanding:)]) {
            [_delegate quadCurveMenuTouchesBeganExpanding:self.expanding];
        }
        return;
    }
