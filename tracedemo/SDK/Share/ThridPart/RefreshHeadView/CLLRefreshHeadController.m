...
//        self.refreshHeadView.timeLabel.text = [NSString stringWithFormat:@"%@：%@", _T(@"HF_Common_Last_Refresh"),destDateString];
//    }
//}
//手动开始刷新
...
- (void)setup
{
    ...
    //self.refreshHeadView.timeLabel.text = _T(@"HF_Common_Refresh_Time");
    self.refreshHeadView.statusLabel.text = _T(@"HF_Common_Down_Refresh");
...
- (void)resetScrollViewContentInsetWithDoneLoadMore {
    ...
    [UIView animateWithDuration:0.3f animations:^{
        ...
        if (_refreshFooterView) {
            ...
            if (self.refreshFooterView.frame.origin.y < kScreenHeight) {
                tmpFrame.origin.y = self.scrollView.frame.size.height;
...
- (CLLRefreshFooterView *)refreshFooterView {
    if (!_refreshFooterView) {
        _refreshFooterView = [[CLLRefreshFooterView alloc] initWithFrame:CGRectMake(0, 1000, CGRectGetWidth([[UIScreen mainScreen] bounds]), CLLRefreshFooterViewHeight)];
...
- (void)setLoadMoreState:(CLLLoadMoreState)loadMoreState {
    switch (loadMoreState) {
        ...
        case CLLLoadMoreStateNormal:{
            //上拉加载更多
            self.refreshFooterView.statusLabel.text = _T(@"HF_Common_Up_Refresh");
        ...
        case CLLLoadMoreStateLoading:{
            //加载中
            self.refreshFooterView.statusLabel.text = _T(@"HF_Common_Refreshing");
    ...
    switch (refreshState) {
        ...
        case CLLRefreshStateLoading: {
            if (self.pullDownRefreshing) {
                self.refreshHeadView.statusLabel.text = _T(@"HF_Common_Now_Refresh");
        ...
        case CLLRefreshStatePulling:
            self.refreshHeadView.statusLabel.text = _T(@"HF_Common_Free_Refresh");
            break;
        default:
            break;
    }
