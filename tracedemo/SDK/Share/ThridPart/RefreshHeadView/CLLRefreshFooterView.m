...
- (UILabel *)statusLabel {
    if (!_statusLabel) {
        _statusLabel = [[UILabel alloc] initWithFrame:CGRectMake(kScreenWidth / 2 - 35,(self.bounds.size.height - 14) * 0.5, 90, 14)];
...
- (UIActivityIndicatorView *)indicatorView {
    if (!_indicatorView) {
        ...
        _indicatorView.frame = CGRectMake(kScreenWidth / 2 - 55,(self.bounds.size.height - 20) * 0.5,20,20);
...
- (void)resetView
{
    if (_indicatorView.isAnimating) {
        ...
        _statusLabel.frame = CGRectMake(kScreenWidth / 2 - 35,(self.bounds.size.height - 14) * 0.5, 90, 14);//我不知道上面一串是做什么的。所以只能把_statusLabel.frame重置为刚开始的frame了
    }else {
        //[_statusLabel sizeToFit];
