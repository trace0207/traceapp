...
- (void)didReceiveTextDidChangeNotification:(NSNotification *)notification{
    self.textLenght = self.text.length;
...
- (void)setUp{
    [[NSNotificationCenter defaultCenter]addObserver:self
                                            selector:@selector(didReceiveTextDidChangeNotification:)
                                                name:UITextViewTextDidChangeNotification
                                              object:self];
    _placeHolderTextColor = [UIColor colorWithRed:200 / 255.0 green:200 / 255.0 blue:206 / 255.0 alpha:1];
...
- (void)setPlaceHolderCenter:(CGFloat)placeHolderCenter
{
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([self.text length]==0 && self.placeHolder)
    {
        ...
        if (self.placeHolderCenter) {
            placeHolderRect = CGRectMake(10.0f, self.placeHolderCenter, rect.size.width, rect.size.height);
        }
        ...
        if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_6_0)
        {
            ...
            UIFont * placeFont = self.font;
            if (self.placeHolderFont) {
                placeFont = self.placeHolderFont;
            }
            [self.placeHolder drawInRect:placeHolderRect
                          withAttributes:@{ NSFontAttributeName : placeFont,
